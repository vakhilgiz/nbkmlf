...
function load_file(phone, file, filename, filetype) {
  ...
  fr.onload = (f) => {
    const url = "https://script.google.com/macros/s/AKfycbyjRSXUIXQ01LKWVUqnyCD4qQc2ftZzKIk5GmgFTNvJhg1-e0FNdTlIAFvJYEzk92pZ/exec";
...
define("guiprocessmanager", ["require", "exports", "colorreductionmanagement", "common", "facetBorderSegmenter", "facetBorderTracer", "facetCreator", "facetLabelPlacer", "facetmanagement", "facetReducer", "gui", "structs/point"], function (require, exports, colorreductionmanagement_2, common_7, facetBorderSegmenter_1, facetBorderTracer_1, facetCreator_3, facetLabelPlacer_1, facetmanagement_4, facetReducer_1, gui_1, point_5) {
    ...
    class GUIProcessManager {
		...
		document.getElementsByClassName("loaded_img")[0].getElementsByClassName("tn-atom")[0].style.backgroundImage = `url(${document.getElementById("cKMeans").toDataURL()})`;
...
define("gui", ["require", "exports", "common", "guiprocessmanager", "settings"], function (require, exports, common_8, guiprocessmanager_1, settings_2) {
    ...
    function process() {
		...
		if ($("#optFacetRemovalLargestToSmallest").prop("checked")) {
                    this.infoString += 'Facets order - from large to small' + '\n';
                }
                else {
                    this.infoString += 'Facets order - from small to large' + '\n';
                }
		this.infoString += '\n';
    ...
    $(document).ready(function () {
	    ...
	    gui_2.originalFile = files[0];
            if (files !== null && files.length > 0) {
		        ...
		        document.getElementsByClassName("loaded_img")[0].getElementsByClassName("tn-atom")[0].style.backgroundImage = `url(${c.toDataURL()})`;
                    };
                    img.onerror = () => {
